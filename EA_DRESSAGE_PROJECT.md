...
3. **Layer 4 — templates** — BUILT + VALIDATED (all 3 tests). `generateCommands(testMoves, events)` → `Command[]`; `renderCommand(cmd, level)` → `string|null`. Levels 1–4. `validate-commands.js` harness.
   - **Known: compound preview label notation** — when a movement has multiple rein-changes (loops, K→X→H figures), `resolveNextMvCtx` returns null and the preview falls back to `nextMvLabel` (e.g. `"C → H · loop H → K"`). The raw test-sheet notation (arrows, middle dots) is visually correct but doesn't read as speech. Future polish: rephrase as spoken text — e.g. `"C → H · loop H → K"` → `"ride to H then loop H to K"`.
